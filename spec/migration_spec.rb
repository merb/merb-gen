...
describe Merb::Generators::Migration do

  before do
    @generator = create_generator(Merb::Generators::Migration, 'SomeMoreStuff')
  end
  ...
  describe '#version' do
    it "should find the current migration version and increase it by one" do
      ...
      Dir.should_receive(:[]).with(app_path('schema/migrations/*')).and_return(@previous_migration_files)

      @generator.version.should == "007"
    end

    it "should be 1 if there are no previous migrations" do
      Dir.should_receive(:[]).with(app_path('schema/migrations/*')).and_return([])
